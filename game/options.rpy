...
define config.name = _("Break Time with Julia")
...
define build.name = "BreakTimeWithJulia"
...
define config.main_menu_music = "songs/nice_morning_loop.mp3"
...
define config.save_directory = "GameDevwithJulia-1773137669"
...
## Deleta arquivos .rpyc ao fechar o jogo (evita cache antigo)
init python:
    def delete_rpyc_on_quit():
        import os
        project = os.path.dirname(renpy.config.gamedir)
        for root, dirs, files in os.walk(project):
            for f in files:
                if f.endswith(".rpyc") or f.endswith(".rpyc.bak"):
                    try:
                        os.remove(os.path.join(root, f))
                    except OSError:
                        pass

    config.quit_callbacks.append(delete_rpyc_on_quit)
